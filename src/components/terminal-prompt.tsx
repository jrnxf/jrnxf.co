...
function Neofetch({ mail }: { mail: string }) {
  return (
    <div className="flex flex-col gap-4 sm:flex-row sm:gap-8">
      <pre className="shrink-0 leading-[1.35] text-accent">{NEOFETCH_ASCII}</pre>
      <div className="flex flex-col gap-0.5">
        <p>
          <span className="text-accent">colby</span>@<span className="text-accent">jrnxf</span>
        </p>
        <p className="text-neutral-600">-----------</p>
        {NEOFETCH_INFO.map(([label, val]) => (
          <p key={label}>
